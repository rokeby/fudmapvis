...
/**
 * Process a new hurricane event
 */
function newHurricane() {
  console.log('Starting a new hurricane simulation');
  // Could add UI notifications here
}

/**
 * Process a new point in the current hurricane path
 */
function newPoint() {
  console.log('New point added to hurricane path');
  drawMap();
}

/**
 * Listen for updates from the API and update visualization
 */
async function listenForNewPoints() {
  try {
    const response = await fetch(VIS_CONFIG.apiEndpoint);
    const data = await response.json();
    
    console.log("Polling for hurricane data, points:", data.length);
    
    // Check if data has changed
    if (JSON.stringify(data) !== JSON.stringify(currentData)) {
      console.log("Hurricane data changed");
      // If fewer points, it's a new hurricane
      if (data.length < currentData.length) {
        currentData = data;
        newHurricane();
      } else {
        // Otherwise it's a new point on the current hurricane
        currentData = data;
        newPoint();
      }
    }
...
// Start polling for updates once the document is ready
window.addEventListener('load', function() {
  console.log("Starting hurricane data polling");
  // Initial call to start
  listenForNewPoints();
  
  // Set up periodic polling
  window.setInterval(listenForNewPoints, VIS_CONFIG.updateInterval);
});
